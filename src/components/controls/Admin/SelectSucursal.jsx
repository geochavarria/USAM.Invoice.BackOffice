...
import { getAllEmpresaAsync, getSucursalByEmpIDAsync } from "@/helpers/backend_helpers/admin_helpers";
...
const SelectSucursal = ({
    ...
    const select_OnLoadAsync = async(ID) => {
        setIsLoading(true);
        setDataSource([]);

        try {
            const response = await getSucursalByEmpIDAsync(ID);
