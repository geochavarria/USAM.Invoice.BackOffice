...
import { getMunicipioByDeptoIDAsync } from "@/helpers/backend_helpers/admin_helpers";
...
const SelectMunicipio = ({
    ...
    const select_OnLoadAsync = async( deptoID ) => {
        setIsLoading(true);
        setOptions([]);

        try {
            const response = await getMunicipioByDeptoIDAsync( deptoID );
