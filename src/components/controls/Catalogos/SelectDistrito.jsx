...
import { getDistritoByMunicipioIDAsync } from "@/helpers/backend_helpers/admin_helpers";
...
const SelectDistrito = ({
    ...
    const select_OnLoadAsync = async( deptoID, munID ) => {
        setIsLoading(true);
        setOptions([]);

        try {
            const response = await getDistritoByMunicipioIDAsync( deptoID , munID );
    ...
    useEffect(()=> {
        select_OnLoadAsync(deptoID, munID)
    },[deptoID, munID])
